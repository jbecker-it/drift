...
data class AppSettings(val ai: AiSettings = AiSettings(), val reminders: ReminderSettings = ReminderSettings(), val sync: SyncSettings = SyncSettings())
...
class SecureSettings(context: Context) {
    ...
    private fun save(name: String, raw: String) { prefs.edit().putString(name, encrypt(raw)).apply(); _state.value = readAll() }
    private fun readAll() = AppSettings(readAi(), readReminders(), readSync())
    ...
    private fun encrypt(text: String): String {
        ...
        val data = cipher.doFinal(text.toByteArray()); val joined = ByteBuffer.allocate(4 + cipher.iv.size + data.size).putInt(cipher.iv.size).put(cipher.iv).put(data).array()
        return Base64.encodeToString(joined, Base64.NO_WRAP)
    }
    private fun decrypt(text: String): String {
        val bytes = ByteBuffer.wrap(Base64.decode(text, Base64.NO_WRAP)); val size = bytes.int; require(size in 12..32)
        val iv = ByteArray(size).also(bytes::get); val cipher = Cipher.getInstance("AES/GCM/NoPadding"); cipher.init(Cipher.DECRYPT_MODE, key(), GCMParameterSpec(128, iv))
    ...
    private fun key(): SecretKey {
        ...
        return KeyGenerator.getInstance(KeyProperties.KEY_ALGORITHM_AES, "AndroidKeyStore").apply {
            init(KeyGenParameterSpec.Builder("drift.settings.v2", KeyProperties.PURPOSE_ENCRYPT or KeyProperties.PURPOSE_DECRYPT)
